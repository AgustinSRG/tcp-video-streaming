...
export function GetAPIURL(path: string): string {
    if (import.meta.env.DEV) {
        return (import.meta.env.VITE_DEV_TEST_HOST || "http://localhost") + path;
    } else {
...
export class Request {
    ...
    public static Do(params: RequestParams): Request {
        if (params.method == "POST") {
            if (params.form) {
                return Request.PostFormData("", params.url, params.form);
            } else {
                return Request.PostJSON("", params.url, params.json || {});
            }
        } else {
